...
def input_producer(data_list, channels, batch_size, need_shuffle):
    ...
    def read_data(data_queue):
        ...
        if False:
            offset = tf.cast(tf.floor(tf.random_uniform([2], 0, IMG_WIDTH - CROP_SIZE + 1)), dtype=tf.int32)
            in_imgproc = tf.image.crop_to_bounding_box(in_imgproc, offset[0], offset[1], CROP_SIZE, CROP_SIZE)
            gt_imgproc = tf.image.crop_to_bounding_box(gt_imgproc, offset[0], offset[1], CROP_SIZE, CROP_SIZE)
...
def gen_PRLNet(inputs, out_channels, is_train=False, reuse=False):
    with tf.variable_scope("PRLNet", reuse=reuse):

        # ------------------------------ Content Aggregation ------------------------------
        # in
        n = Conv2d(inputs, 64, filter_size=3, strides=1, padding='SAME', name='in/k7n64s1')
        ...
        n = Conv2d(n, out_channels, filter_size=3, strides=1, act=None, padding='SAME', name='latent')
        #content_map = tf.nn.tanh(n)
        content_map = n
